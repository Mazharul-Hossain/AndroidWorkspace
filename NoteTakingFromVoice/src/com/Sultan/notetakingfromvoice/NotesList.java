...
public class NotesList extends ListActivity implements
	...
	public void onResume() {
		...
		mAccount = new GoogleAccountManager(this)
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		/*
		 * Generate any additional actions that can be performed on the overall
		 * list. In a normal install, there are no additional actions found
		 * here, but this allows other applications to extend our menu with
		 * their own actions.
		 */
	...
	public boolean onPrepareOptionsMenu(Menu menu) {
		...
		final boolean haveItems = getListAdapter() != null
				&& getListAdapter().getCount() > 0;

		/*
		 * If there are any notes in the list (which implies that one of them is
		 * selected), then we need to generate the actions that can be performed
		 * on the current selection. This will be a combination of our own
		 * specific actions along with any extensions that can be found.
		 */
		if (haveItems) {
			 ...
			 * to send an Intent based on the selected menu item.
			 */
	...
	public void onCreateContextMenu(ContextMenu menu, View view,
		...
		/*
		 * Append to the menu items for any other activities that can do stuff
		 * with it as well. This does a query on the system for any activities
		 * that implement the ALTERNATIVE_ACTION for our data, adding a menu
		 * item for each one that is found.
		 */
	...
	public boolean onContextItemSelected(MenuItem item) {
		...
		// Appends the selected note's ID to the URI sent with the incoming
		// Intent.
		...
		case R.id.context_delete:
			 ...
			 * format. Please see the introductory note about performing
			 * provider operations on the UI thread. Set the note deleted status
			 * to 1.
			 */
	...
	protected void onListItemClick(ListView l, View v, int position, long id) {
		...
		if (Intent.ACTION_PICK.equals(action)
			 ...
			 * ACTION_EDIT. The Intent's data is the note ID URI. The effect is
			 * to call NoteEdit.
			 */
	...
	/**
	 * This method is called when the OAuthManager has successfully authorized
	 * the application for the requested scopes.
	 */
	private void loadNotes() {
		...
		intent.setData(Uri.parse("content://com.google.provider.NotePad/"
				+ mAccount.name + "/notes/"));

		getLoaderManager().destroyLoader(0);
		getLoaderManager().initLoader(0, null, this);
		if (mProgressBar == null) {
			mProgressBar = ProgressDialog.show(this, null, "Loading notes...",
					true);
		}
	}
	...
	public Loader<Cursor> onCreateLoader(int id, Bundle bundle) {
		...
		return new CursorLoader(this, getIntent().getData(), PROJECTION,
				NotePad.Notes.COLUMN_NAME_DELETED + " = 0" /* where */,
				null /* columns */, NotePad.Notes.DEFAULT_SORT_ORDER);
	}
