...
public class DriveSyncer1 {
	 ...
	 */
	public DriveSyncer1(Context context, ContentProviderClient provider,
			Account account) {
		mContext = context;
		mProvider = provider;
		mAccount = account;
		...
		if (mLargestChangeId == -1) {
			...
			try {
				Cursor cursor = mProvider.query(uri, PROJECTION,
						NotePad.Notes.COLUMN_NAME_FILE_ID + " IS NOT NULL",
						null, null);
	...
	private void performFullSync() {
		...
		try {
			...
			About about = mService.about().get().setFields(ABOUT_GET_FIELDS)
					.execute();
