...
public class DriveSyncer {
	...
	private static final String[] PROJECTION = new String[] { NotePad.Notes._ID,
			...
			NotePad.Notes.COLUMN_NAME_FILE_ID, NotePad.Notes.COLUMN_NAME_DELETED };
	...
	public void performSync() {
		...
		if (mLargestChangeId == -1) {
			...
			try {
				Cursor cursor = mProvider.query(uri, PROJECTION,
						NotePad.Notes.COLUMN_NAME_FILE_ID + " IS NOT NULL", null, null);
		...
		if (localFileModificationDate > driveFile.getModifiedDate().getValue()) {
			try {
				if (localFile.getShort(COLUMN_INDEX_DELETED) != 0) {
					...
					values.put(NotePad.Notes.COLUMN_NAME_MODIFICATION_DATE, updatedFile
	...
	private Drive getDriveService() {
		...
		if (mService == null) {
			try {
				String authToken = GoogleAuthUtil.getTokenWithNotification(mContext,
						mAccount.name, "oauth2:" + DriveScopes.DRIVE_FILE, null,
						NotePad.AUTHORITY, null);
				 ...
				 * credential.setSelectedAccountName(mAccount.name); // Trying
				 * to get a token right away to see if we are authorized
				 * credential.getToken();
				 */
				credential.setAccessToken(authToken);
	 ...
	 * 
	 * @param string
	 *            Data to compute the MD5 checksum from.
