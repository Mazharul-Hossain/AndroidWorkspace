...
package com.mazhar.browserhistory;
...
public class Browser {
	...
	public static final Uri BOOKMARKS_URI = Uri
	...
	public static final String[] HISTORY_PROJECTION = new String[] {
			...
			BookmarkColumns.DATE, BookmarkColumns.BOOKMARK,
			BookmarkColumns.TITLE, BookmarkColumns.FAVICON,
			BookmarkColumns.THUMBNAIL, BookmarkColumns.TOUCH_ICON,
	...
	public static final Uri SEARCHES_URI = Uri
			.parse("content://browser/searches");
	...
	/*
	 * Set a cap on the count of history items in the history/bookmark table, to
	 * prevent db and layout operations from dragging to a crawl. Revisit this
	 * cap when/if db/layout performance improvements are made. Note: this does
	 * not affect bookmark entries -- if the user wants more bookmarks than the
	 * cap, they get them.
	 */
	private static final int MAX_HISTORY_COUNT = 250;

	/**
	 * Open the AddBookmark activity to save a bookmark. Launch with and/or url,
	 * which can be edited by the user before saving.
	 * 
	 * @param c
	 *            Context used to launch the AddBookmark activity.
	 * @param title
	...
	/*
	 * static final void sendString(Context c, String s) { sendString(c, s,
	 * c.getString(com.android.internal.R.string.sendText)); }
	 */

	/**
	 * Find an application to handle the given string and, if found, invoke it
	 * with the given string as a parameter.
	 * 
	 * @param c
	 *            Context used to launch the new activity.
	 * @param stringToSend
	 *            The string to be handled.
	 * @param chooserDialogTitle
	 *            The title of the dialog that allows the user to select between
	 *            multiple applications that are all capable of handling the
	 *            string.
	 * @hide pending API council approval
	 */
	public static final void sendString(Context c, String stringToSend,
	...
	/**
	 * Return a cursor pointing to a list of all the bookmarks. Requires
	...
	public static final Cursor getAllBookmarks(ContentResolver cr)
		...
		return cr.query(BOOKMARKS_URI, new String[] { BookmarkColumns.URL },
				"bookmark = 1", null, null);
	}

	/**
	 * Return a cursor pointing to a list of all visited site urls. Requires
	 * {@link android.Manifest.permission#READ_HISTORY_BOOKMARKS}
	 * 
	 * @param cr
	 *            The ContentResolver used to access the database.
	...
	/**
	 * Return a Cursor with all history/bookmarks that are similar to url, where
	 * similar means 'http(s)://' and 'www.' are optional, but the rest of the
	 * url is the same.
	 * 
	 * @param cr
	 *            The ContentResolver used to access the database.
	...
	public static final Cursor getVisitedLike(ContentResolver cr, String url) {
		...
		if (secure) {
			...
			DatabaseUtils.appendEscapedSQLString(whereClause, "https://"
					+ compareString);
			addOrUrlEquals(whereClause);
			DatabaseUtils.appendEscapedSQLString(whereClause, "https://www."
					+ compareString);
		} else {
			whereClause = new StringBuilder(BookmarkColumns.URL + " = ");
			...
			addOrUrlEquals(whereClause);
			DatabaseUtils.appendEscapedSQLString(whereClause, "http://"
					+ compareString);
			addOrUrlEquals(whereClause);
			DatabaseUtils.appendEscapedSQLString(whereClause, "http://"
					+ wwwString);
	 ...
	 * @param real
	 *            If true, this is an actual visit, and should add to the number
	 *            of visits. If false, the user entered it manually.
	 */
	public static final void updateVisitedHistory(ContentResolver cr,
		...
		try {
			...
			if (c.moveToFirst()) {
				...
				if (real) {
					map.put(BookmarkColumns.VISITS,
							c.getInt(HISTORY_PROJECTION_VISITS_INDEX) + 1);
				} else {
					map.put(BookmarkColumns.USER_ENTERED, 1);
				}
				map.put(BookmarkColumns.DATE, now);
				String[] projection = new String[] { Integer.valueOf(
						c.getInt(0)).toString() };
	...
	/**
	 * Returns all the URLs in the history. Requires
	 * {@link android.Manifest.permission#READ_HISTORY_BOOKMARKS}
	 * 
	 * @param cr
	 *            The ContentResolver used to access the database.
	...
	/**
	 * If there are more than MAX_HISTORY_COUNT non-bookmark history items in
	 * the bookmark/history table, delete TRUNCATE_N_OLDEST of them. This is
	 * used to keep our history table to a reasonable size. Note: it does not
	 * prune bookmarks. If the user wants 1000 bookmarks, the user gets 1000
	 * bookmarks. Requires
	 * {@link android.Manifest.permission#READ_HISTORY_BOOKMARKS} Requires
	...
	public static final void truncateHistory(ContentResolver cr) {
		...
		try {
			...
			c = cr.query(BOOKMARKS_URI, TRUNCATE_HISTORY_PROJECTION,
					"bookmark = 0", null, BookmarkColumns.DATE);
	...
	public static final boolean canClearHistory(ContentResolver cr) {
		...
		boolean ret = false;
		try {
			c = cr.query(BOOKMARKS_URI, new String[] { BookmarkColumns._ID,
					BookmarkColumns.BOOKMARK, BookmarkColumns.VISITS },
					"bookmark = 0 OR visits > 0", null, null);
			ret = c.moveToFirst();
		} catch (IllegalStateException e) {
	...
	/**
	 * Helper function to delete all history items and revert all bookmarks to
	 * zero visits which meet the criteria provided. Requires
	 * {@link android.Manifest.permission#READ_HISTORY_BOOKMARKS} Requires
	 * {@link android.Manifest.permission#WRITE_HISTORY_BOOKMARKS}
	 * 
	 * @param cr
	 *            The ContentResolver used to access the database.
	 * @param whereClause
	 *            String to limit the items affected. null means all items.
	 */
	private static final void deleteHistoryWhere(ContentResolver cr,
			String whereClause) {
		Cursor c = null;
		try {
			c = cr.query(BOOKMARKS_URI, HISTORY_PROJECTION, whereClause, null,
					null);
			if (c.moveToFirst()) {
				...
				do {
					...
					boolean isBookmark = c
							.getInt(HISTORY_PROJECTION_BOOKMARK_INDEX) == 1;
					if (isBookmark) {
						if (firstTime) {
							firstTime = false;
						} else {
	...
	public static final void addSearchUrl(ContentResolver cr, String search) {
		...
		Cursor c = null;
		try {
			c = cr.query(SEARCHES_URI, SEARCHES_PROJECTION,
					SEARCHES_WHERE_CLAUSE, new String[] { search }, null);
			ContentValues map = new ContentValues();
	...
	/**
	 * Request all icons from the database. This call must either be called in
	 * the main thread or have had Looper.prepare() invoked in the calling
	 * thread. Requires
	 * {@link android.Manifest.permission#READ_HISTORY_BOOKMARKS}
	 * 
	 * @param cr
	 *            The ContentResolver used to access the database.
	 * @param where
	 *            Clause to be used to limit the query from the database. Must
	 *            be an allowable string to be passed into a database query.
	 * @param listener
	 *            IconListener that gets the icons once they are retrieved.
	 */

	/*
	 * public static final void requestAllIcons(ContentResolver cr, String
	 * where, WebIconDatabase.IconListener listener) {
	 * WebIconDatabase.getInstance().bulkRequestIconForPageUrl(cr, where,
	 * listener); }
	 */
