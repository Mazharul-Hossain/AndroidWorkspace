...
public class Login extends Activity {
	...
	public void login() {
		...
		if (!u_name.isEmpty() && !u_pass.isEmpty()) {
			...
			try {
				json.put("email", u_name);
				json.put("password", u_pass);
				...
				if (!(jArray == null) && jArray.length() > 0) {
					...
					Log.d("log_tag", "Done ca[turing data");

					json_data = jArray.getJSONObject(0);

					String flag = json_data.getString("flag");
					if (flag.equalsIgnoreCase("true")) {
						Log.d("log_tag", "Done ca[turing data1");
						u_name1 = json_data.getString("email");
						Log.d("log_tag", "Done ca[turing data2");
						appState.setState(u_name1);
						Log.d("log_tag", appState.getState());
					...
					Log.d("log_tag", "Done ca[turing data3");
				}
				Log.d("log_tag", "Done ca[turing data4");
				if (!appState.getState().equalsIgnoreCase("login")) {
					/*
					 * Toast.makeText(getApplicationContext(), "You have login "
					 * + appState.getName(), Toast.LENGTH_LONG).show();
					 */

					Log.d("log_tag", "Done ca[turing data5");
					finish();
				} else {
