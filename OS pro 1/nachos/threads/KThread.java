...
public class KThread {
	...
	public KThread(Runnable target) {
		this();
		this.target = target;
		semaphoreforjoin = new Semaphore(0);
	}
	 ...
	 */
	public void fork() {
		Lib.assert(status == statusNew);// only a new thread can be forked
	...
	public static void finish() {
		...
		currentThread.semaphoreforjoin.V();
	...
	private static class PingTest implements Runnable {
		...
		public void run() {
			...
			KThread cth1 = new KThread(new ChildrenThread(1));
			KThread cth2 = new KThread(new ChildrenThread(2));

			cth1.setName("children  thread 1");
			cth2.setName("children  thread 2");
			...
			for (int i = 0; i < 6; i++) {
				System.out.println("parent thread " + which
						+ " executing loop " + i + " times");
				currentThread.yield();
				if (i == 1) {
	...
	public static void selfTest() {
		...
		new KThread(new PingTest(0)).setName("parent thread").fork();
	...
	/**
	 * This Seamaphore will be used for thread join initial value of this
	 * semaphore is 0 and join will wait on it as it has 0 vale finish method
	 * will increase the value of this semaphore and notify join
	 */
	Semaphore semaphoreforjoin;
	...
	/**
	 * This Class Test The Join property of the nachos Thread A new Thread Is
	 * opened Inside a thread
	 */
	static class ChildrenThread implements Runnable {
		private final int which;

		ChildrenThread(int num) {
			which = num;
		...
		public void run() {
			...
			KThread gcth1 = new KThread(new GrandChildrenThread(1, which));

			gcth1.setName("grand child  thread " + which);

			gcth1.fork();

			for (int j = 0; j < 12; j++) {
				System.out.println("children thread " + which
						+ " executing loop " + j + " times");
				currentThread.yield();
				if (j == 4) {
					gcth1.join();
				}
	...
	static class GrandChildrenThread implements Runnable {
		private final int which, cwhich;

		GrandChildrenThread(int num, int nm1) {
			which = num;
			cwhich = nm1;
		...
		public void run() {
			...
			for (int j = 0; j < 10; j++) {
				System.out.println("grand children thread " + which
						+ "under children thread " + cwhich
						+ " executing loop " + j + " times");
				currentThread.yield();
