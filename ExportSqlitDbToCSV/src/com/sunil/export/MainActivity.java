...
public class MainActivity extends Activity implements OnClickListener {
	...
	public void onClick(View arg0) {
		...
		if (arg0 == btnexport) {
				...
				try {
					while ((nextLine = reader.readNext()) != null) {
						...
						if (fname.equalsIgnoreCase("First Name")) {
							...
							int value = dbhelper.addPersonData(new Person(fname, lname,
									address, email));
							if (value == 1) {
								Toast.makeText(getApplicationContext(),
										"Data inerted into table", Toast.LENGTH_LONG)
										.show();
							}
						}
			...
			if (to.length() < 1) {
				Toast.makeText(getApplicationContext(), "Please Enter Recipent Email",
						Toast.LENGTH_LONG).show();
	...
	private class ExportDatabaseCSVTask extends AsyncTask<String, Void, Boolean> {
			...
			try {
				...
				if (listdata.size() > 1) {
					for (int index = 0; index < listdata.size(); index++) {
						...
						String arrStr[] = { person.getFirtname(), person.getLastname(),
		...
		protected void onPostExecute(final Boolean success) {
			...
			if (success) {
				...
				Toast.makeText(MainActivity.this, "Export failed!", Toast.LENGTH_SHORT)
						.show();
			}
		}
	}
