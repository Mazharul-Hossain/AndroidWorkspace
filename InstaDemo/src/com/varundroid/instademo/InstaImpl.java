package com.varundroid.instademo;
...
import com.varundroid.instademo.InstaActivity.ResponseListener;
...
public class InstaImpl {
	...
	public InstaImpl(Context context) {
		...
		mClient_id = context.getResources().getString(R.string.instagram_id); // Recommended:
																				...
																				// string
																				// class
		mClient_secret = context.getResources().getString(R.string.instagram_secret); // Recommended:
																						// Put
																						// your
																						// Instagram
																						// Secret
		...
		mAuthURLString = AUTHURL + "?client_id=" + mClient_id + "&redirect_uri="
				+ CALLBACKURL
				+ "&response_type=code&display=touch&scope=likes+comments+relationships";
		mTokenURLString = TOKENURL + "?client_id=" + mClient_id + "&client_secret="
				+ mClient_secret + "&redirect_uri=" + CALLBACKURL
		...
		InstaLoginDialog instaLoginDialog = new InstaLoginDialog(context, mAuthURLString,
	...
	public class InstaAuthDialogListener implements
			com.varundroid.instademo.InstaLoginDialog.AuthDialogListener {
	...
	public class GetInstagramTokenAsyncTask extends AsyncTask<Void, Void, Void> {
		...
		protected Void doInBackground(Void... params) {
			try {
				...
				outputStreamWriter.write("client_id=" + mClient_id + "&client_secret="
						+ mClient_secret + "&grant_type=authorization_code"
						+ "&redirect_uri=" + CALLBACKURL + "&code=" + mToken);
				...
				// Log.e(TAG, name);
				mSessionStore
						.saveInstagramSession(mAccessTokenString, id, username, name);
