...
public class AutoCompleteTextView extends EditText implements
	...
	private final Drawable mDropDownListHighlight;
	...
	private final Runnable mResizePopupRunnable = new ResizePopupRunnable();
	...
	public AutoCompleteTextView(Context context, AttributeSet attrs,
		...
		TypedArray a = context.obtainStyledAttributes(attrs,
				com.android.internal.R.styleable.AutoCompleteTextView,
				defStyle, 0);
		...
		mDropDownListHighlight = a
				.getDrawable(R.styleable.AutoCompleteTextView_dropDownSelector);
		mDropDownVerticalOffset = (int) a.getDimension(
				R.styleable.AutoCompleteTextView_dropDownVerticalOffset, 0.0f);
		mDropDownHorizontalOffset = (int) a
				.getDimension(
		...
		mDropDownHeight = a.getLayoutDimension(
				R.styleable.AutoCompleteTextView_dropDownHeight,
				ViewGroup.LayoutParams.WRAP_CONTENT);

		mHintResource = a.getResourceId(
				R.styleable.AutoCompleteTextView_completionHintView,
		...
		if (isPopupShowing()) {
			...
			if (keyCode != KeyEvent.KEYCODE_SPACE
				...
				if (adapter != null) {
					...
					firstItem = allEnabled ? 0 : mDropDownList
							.lookForSelectablePosition(0, true);
	...
	/**
	 * Returns <code>true if the amount of text in the field meets
	 ...
	 * filter with a different pattern, for instance a substring of <code>text.
	 * 
	 * 
	 * @param text
		...
		if (isPopupShowing()) {
			...
			if (mItemClickListener != null) {
				...
				mItemClickListener.onItemClick(list, null,
						completion.getPosition(), completion.getId());
			}
		}
	}
	...
	private void performCompletion(View selectedView, int position, long id) {
		if (isPopupShowing()) {
			...
			if (mItemClickListener != null) {
				...
				mItemClickListener
						.onItemClick(list, selectedView, position, id);
			}
		}
	 ...
	 * 
	 * @param filter
	 *            If <code>false, no filtering will be performed
	...
	public void ensureImeVisible(boolean visible) {
		mPopup.setInputMethodMode(visible ? PopupWindow.INPUT_METHOD_NEEDED
	...
	private View getHintView(Context context) {
		if (mHintText != null && mHintText.length() > 0) {
			final TextView hintView = (TextView) LayoutInflater.from(context)
					.inflate(mHintResource, null)
	 ...
	 * <code>null if {@link #setAdapter} was not called with
	 * a Filterable.
	 */
	...
	private class PopupDataSetObserver extends DataSetObserver {
		...
		public void onChanged() {
			if (isPopupShowing()) {
				...
				// change. We can't do it from here, because we are in the
				// middle
