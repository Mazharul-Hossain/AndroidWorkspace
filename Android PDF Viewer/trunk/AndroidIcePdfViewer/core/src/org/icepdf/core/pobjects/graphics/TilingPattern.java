...
public class TilingPattern extends Stream implements Pattern {
    ...
    // Fill colour
    public int fillColour = Color.TRANSPARENT;

    //  initiated flag
    private boolean inited;

    // textured paint
    // TODO [FHe] texture paint
//    private TexturePaint texturePaint;
     ...
     */
    private static Matrix getAffineTransform(Vector v) {
        float f[] = new float[6];
        for (int i = 0; i < 6; i++) {
            f[i] = ((Number) v.elementAt(i)).floatValue();
        }
        return Util.createMatrix(f);
    }
    ...
    public int getFirstColor() {
        ...
        if (shapes != null && unColored == Color.TRANSPARENT) {
            for (int i = 0, max = shapes.shapes.size(); i < max; i++) {
                if (shapes.shapes.get(i) instanceof Integer) {
                    unColored = ((Integer)shapes.shapes.get(i)).intValue();
                    return unColored;
                }
            }
        }
        // if now shapes then we go with black.  
        if (unColored == Color.TRANSPARENT) {
