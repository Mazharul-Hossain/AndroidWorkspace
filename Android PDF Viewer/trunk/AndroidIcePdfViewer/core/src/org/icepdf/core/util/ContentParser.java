...
public class ContentParser {
	...
	private static final String TAG = "ANDICEPDF.ContentParser";
...
//    private static HashTable tokenFrequency = new Hashtable(90);
    ...
    public Shapes parse(InputStream source) {
        ...
        // preserve colour and fill data for the XOjbects content stream.
            ...
            graphicState.setCTM(new Matrix());
...
//                        collectTokenFrequency(PdfOps.c_TOKEN);
                        ...
                        geometricPath.cubicTo(x1, y1, x2, y2, x3, y3);
                    }
...
//                        collectTokenFrequency(PdfOps.f_TOKEN);
                        if (geometricPath != null) {
                        	// TODO [FHe] GeneralPath.WIND_NON_ZERO = FillType.WINDING?
                            geometricPath.setFillType(FillType.WINDING);
...
//                        collectTokenFrequency(PdfOps.Do_TOKEN);
                        ...
                        // Image XObject
                            ...
                            Bitmap im = resources.getImage(xobjectName,
...
//                        collectTokenFrequency(PdfOps.f_STAR_TOKEN);
                        if (geometricPath != null) {
                            ...
                            geometricPath.setFillType(FillType.EVEN_ODD);
                            commonFill(shapes, geometricPath);
                        }
                        geometricPath = null;
                    }
...
//                        collectTokenFrequency(PdfOps.W_TOKEN);
                        if (geometricPath != null) {
                            ...
                            geometricPath.computeBounds(bounds, false);
                            Rect r = new Rect();
                            bounds.round(r);
                            graphicState.setClip(r);
                        }
...
//                        collectTokenFrequency(PdfOps.y_TOKEN);
                        ...
                        float y1 = ((Number) stack.pop()).floatValue();
                        float x1 = ((Number) stack.pop()).floatValue();
                        geometricPath.cubicTo(x1, y1, x3, y3, x3, y3);
                    }
...
//                        collectTokenFrequency(PdfOps.d0_TOKEN);
                        ...
                        textState.setType3HorizontalDisplacement(new PointF(x, y));
                    }
...
//        System.out.println("Extraction Duration " + (endTime - startTime));
    ...
    void parseText(Parser parser, Shapes shapes,
        ...
        PointF advance = new PointF(0, 0);
        textBlockBase = new Matrix(graphicState.getCTM());

        // transformation matrix used to cMap core space to drawing space
        graphicState.getTextState().tmatrix = new Matrix();
        graphicState.getTextState().tlmatrix = new Matrix();
        graphicState.scale(1, -1);
...
//                    collectTokenFrequency(PdfOps.Tm_TOKEN);
                    ...
                    Matrix af = new Matrix(textBlockBase);
                    graphicState.getTextState().tmatrix = net.sf.andpdf.utils.Util.createMatrix(f1, f2, f3, f4, f5, f6);
                    af.preConcat(graphicState.getTextState().tmatrix);
                    graphicState.set(af);
                    graphicState.scale(1, -1);
                    // add a new line for text extraction
//                    if (extractText){
...
//                    collectTokenFrequency(PdfOps.SINGLE_QUOTE_TOKEN);
                    ...
                    PointF d = (PointF) drawString(
                            stringObject.getLiteralStringBuffer(
                                    textState.font.getSubTypeFormat(),
                                    textState.font.getFont()),
                            new PointF(0, 0), 0, 0, graphicState.getTextState(),
                            shapes, extractText, extractedText);
                    graphicState.translate(d.x, 0);
                    shift += d.x;
                    if (extractText) {
...
//                    collectTokenFrequency(PdfOps.DOUBLE_QUOTE__TOKEN);
    ...
    void parseInlineImage(Parser p, Shapes shapes) throws IOException {
        try {
            ...
            Bitmap im = st.getImage(graphicState.getFillColor(), resources, true);
            st.dispose(false);
            Matrix af = new Matrix(graphicState.getCTM());
            graphicState.scale(1, -1);
            graphicState.translate(0, -1);
            shapes.add(im);
            graphicState.set(af);
...
//        collectTokenFrequency(PdfOps.G_TOKEN);
        ...
        graphicState.setStrokeColor(Color.argb(0xff, (int)(gray*0xff), (int)(gray*0xff), (int)(gray*0xff)));
    }
...
//        collectTokenFrequency(PdfOps.g_TOKEN);
        ...
        graphicState.setFillColorSpace(
                PColorSpace.getColorSpace(library, new Name("DeviceGray")));
        graphicState.setFillColor(Color.argb(0xff, (int)(gray*0xff), (int)(gray*0xff), (int)(gray*0xff)));
    }
...
//        collectTokenFrequency(PdfOps.RG_TOKEN);
        ...
        graphicState.setStrokeColor(Color.argb(0xff,(int)(r*0xff), (int)(gg*0xff), (int)(b*0xff)));
    }
...
//        collectTokenFrequency(PdfOps.rg_TOKEN);
        ...
        graphicState.setFillColorSpace(
                PColorSpace.getColorSpace(library, new Name("DeviceRGB")));
        graphicState.setFillColor(Color.argb(0xff,(int)(r*0xff), (int)(gg*0xff), (int)(b*0xff)));
    }
...
//        collectTokenFrequency(PdfOps.Q_TOKEN);
    ...
    private static void consume_cm(GraphicsState graphicState, Stack stack,
                                   boolean inTextBlock, Matrix textBlockBase) {
//        collectTokenFrequency(PdfOps.cm_TOKEN);
        ...
        if (!inTextBlock) {
            ...
            af.preConcat(Util.createMatrix(a, b, c, d, e, f));
            // add the transformation to the graphics state
            graphicState.set(af);
            // update the clip, translate by this CM
            graphicState.updateClipCM(Util.createMatrix(a, b, c, d, e, f));
        } else {
            // update the textBlockBase with the cm matrix
            textBlockBase.preConcat(Util.createMatrix(a, b, c, d, e, f));
        }
    }
...
//        collectTokenFrequency(PdfOps.Ts_TOKEN);
    ...
    private PointF drawString(
        ...
        float advanceX = ((PointF) advance).x;
        float advanceY = ((PointF) advance).y;
        ...
        float firstCharWidth = (float) textState.currentfont.echarAdvance(currentChar).x;
...
//        System.out.println("-> " + displayText + " " + whiteSpace);
        ...
        for (int i = 0; i < textLength; i++) {
            ...
            if (!textState.font.isAFMFont()) {
                newAdvanceX = (float) currentFont.echarAdvance(currentChar).x;
            } else {
                // Problematic Type1 characters that hare hard to draw.
                if (currentChar != 160 && currentChar > 31) {
                    newAdvanceX = (float) currentFont.echarAdvance(currentChar).x;
                } else {
                    newAdvanceX = (float) currentFont.echarAdvance(' ').x;
                }
            }
...
//        System.out.println("RMode " + rmode);
        ...
        return new PointF(advanceX, advanceY);
    }
    ...
    private void drawModeStroke(TextSprite textSprites, TextState textState,
        ...
        textState.tmatrix.getValues(values);
     ...
     */
    private void commonStroke(Shapes shapes, Path geometricPath) {
...
//            alpha = 0.1f;
     ...
     * @param geometricPath current path.
     */
    private void commonFill(Shapes shapes, Path geometricPath) {
...
//                graphicState.getDashPhase()));
    ...
    private static void applyTextScaling(GraphicsState graphicState) {
        ...
        Matrix horizontalScalingTransform =
                Util.createMatrix(graphicState.getTextState().hScalling,
                        0, 0, 1, 0, 0);
        // get the current CTM
        Matrix af = new Matrix(graphicState.getCTM());
        // do the matrix concatenation math
        af.preConcat(horizontalScalingTransform);
        // add the transformation to the graphics state
        graphicState.set(af);
    }
