...
public class CCITTFax {
    ...
    private static int findPositionInTable(Code lookFor, Code[][] lookIn) {
        ...
        if (lookInWithSameLength == null)
            return -1;
        int len = lookInWithSameLength.length;
        for (int i = 0; i < len; i++) {
            Code potentialMatch = lookInWithSameLength[i];
...
//    }
    ...
    static int findWhite(BitStream inb, Code code) throws IOException {
        ...
        while (!inb.atEndOfFile()) {
            ...
            code.append(i != 0);
            int j;
            j = findPositionInTable(code, twcodes);
            if (j >= 0) {
                //System.err.println("BINGO! tw "+_twcodes[j]+" "+j);
                return j;
            }
            j = findPositionInTable(code, mwcodes);
            if (j >= 0) {
                //System.err.println("BINGO! mw "+_mwcodes[j]+" "+(j+1)*64);
                return (j + 1) * 64;
            }
    ...
    static int findBlack(BitStream inb, Code code) throws IOException {
        ...
        while (!inb.atEndOfFile()) {
            ...
            code.append(i != 0);
            int j;
            j = findPositionInTable(code, tbcodes);
            if (j >= 0) {
                //System.err.println("BINGO! tb "+_tbcodes[j]+" "+j);
                return j;
            }
            j = findPositionInTable(code, mbcodes);
            if (j >= 0) {
    ...
    public static Bitmap attemptDeriveBufferedImageFromBytes(
            Stream stream, Library library, Hashtable streamDictionary, int fill) {
...
//if( k < 0 ) return null; //TODO 4243: Temporarily disable Group4 JAI
        ...
        if (!hasHeader) {
            ...
            if (img == null) {
                for (int i = 1; i <= 2; i++) { // Try the three other types of compression (1, 2, 3, 4)
                    compression++;
     ...
     */
    private static Bitmap deriveBufferedImageFromTIFFBytes(
            InputStream in, Library library, int compressedBytes, int width, int height) {
    	Bitmap img = null;
        try {
        	// TODO [FHe] decode tiff
        	if (true) throw new Exception("deocde tiff not yet supported");
...
//            }
    ...
    private static Bitmap applyImageMaskAndDecodeArray(Bitmap img, boolean imageMask, Boolean blackIs1, Vector decode, int fill) {
    	// TODO [FHe] support color models
