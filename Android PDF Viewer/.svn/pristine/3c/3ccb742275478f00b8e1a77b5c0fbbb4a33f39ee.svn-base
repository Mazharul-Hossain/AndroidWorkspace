...
public class FontUtil {
    ...
    /**
     * Utitility method which maps know sytle strings to an Android font style constants.  The style
     * attribute read as follows fron the Typeface constructor:
     * <ul>
     ...
     */
    public static int guessAndroidFontStyle(String name) {
        name = name.toLowerCase();
        int decorations = 0;
        if ((name.indexOf("boldital") > 0) || (name.indexOf("demiital") > 0)) {
            decorations |= Typeface.BOLD | Typeface.ITALIC;
        } else if (name.indexOf("bold") > 0 || name.indexOf("black") > 0 || name.indexOf("demi") > 0) {
            decorations |= Typeface.BOLD;
        } else if (name.indexOf("ital") > 0 || name.indexOf("obli") > 0) {
            decorations |= Typeface.ITALIC;
        } else {
            decorations |= Typeface.NORMAL;
        }
        return decorations;
    }
