...
public class GraphicsState {
    ...
    private Join lineJoin = Join.MITER;
    ...
    public GraphicsState(GraphicsState parentGraphicsState) {
        ...
        CTM = new Matrix(parentGraphicsState.CTM);
        ...
        fillColor = parentGraphicsState.fillColor;

        strokeColor = parentGraphicsState.strokeColor;
        shapes = parentGraphicsState.shapes;
        if (parentGraphicsState.clip != null) {
            clip = new Rect(parentGraphicsState.clip);
        }
    ...
    public GraphicsState restore() {
        ...
        if (parentGraphicState != null) {
            ...
            if (parentGraphicState.clip != null) {
                parentGraphicState.shapes.add(new Rect(parentGraphicState.clip));
...
//            parentGraphicState.shapes.add(parentGraphicState.getStrokeColor());
     ...
     */
    public void updateClipCM(Matrix af) {
        ...
        if (clip != null) {
            ...
            Matrix afInverse = new Matrix();
            if (!af.invert(afInverse))
                logger.log(Level.FINE, "Eror generating clip inverse.");

            // transform the clip.
            RectF rf = new RectF(clip); 
            afInverse.mapRect(rf);
            clip.set((int)rf.left, (int)rf.top, (int)rf.right, (int)rf.bottom);
        }
    }
     ...
     */
    public void setClip(Rect newClip) {
        if (newClip != null) {
            ...
            clip = new Rect(newClip);

            // add new clip shape to stack
            shapes.add(new Rect(clip));
    ...
    public void setLineCap(int lineCapNum) {
    	switch (lineCapNum) {
    	case (0): {
    		lineCap = Cap.BUTT;
    		break;
    ...
    public void setLineJoin(int lineJoinNum) {
    	switch (lineJoinNum) {
    	case (0): {
    		lineJoin = Join.MITER;
    		break;
