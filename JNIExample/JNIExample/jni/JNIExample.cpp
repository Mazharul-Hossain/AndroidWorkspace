...
JNIEXPORT jobject JNICALL Java_org_wooyd_android_JNIExample_JNIExampleInterface_getNewData(
	...
	jmethodID dataConstructor = env->GetMethodID(dataClass, "<init>",
...
JNIEXPORT jstring JNICALL Java_org_wooyd_android_JNIExample_JNIExampleInterface_getDataString(
	...
	jfieldID dataStringField = env->GetFieldID(dataClass, "s",
			"Ljava/lang/String;");
	if (!dataStringField) {
		LOGE("getDataString: failed to get field ID");
		return NULL;
	}
	jstring dataStringValue = (jstring) env->GetObjectField(dataObject,
			dataStringField);
