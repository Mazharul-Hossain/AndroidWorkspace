...
public class NoteEditor extends Activity implements
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (Intent.ACTION_EDIT.equals(action)) {
			...
			if (mFileId == null || mFileId.length() == 0) {
				...
				Intent accountPickerIntent = AccountPicker
						.newChooseAccountIntent(null, null,
								Preferences.PreferencesFragment.ACCOUNT_TYPE,
			...
			Log.e(TAG, "Unknown action, exiting: " + action + " (uri: "
					+ intent.getData() + ")");
			finish();
			return;
		}
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		case CHOOSE_ACCOUNT:
			...
			if (mAccountName != null && mAccountName.length() > 0) {
				...
				mUri = Uri.parse("content://com.google.provider.NotePad/"
						+ mAccountName + "/files/" + mFileId);
				mState = STATE_EDIT;
			} else {
	...
	protected void onResume() {
		...
		if (mState == STATE_EDIT) {
			...
			if (mProgressBar == null) {
				mProgressBar = ProgressDialog.show(this, null,
						"Loading note...", true);
			}
		}
	}
	...
	public void onLoadFinished(Loader<Cursor> loader, Cursor cursor) {
		...
		if (mCursor != null && mCursor.getCount() > 0) {
			...
			if (mCursor.getLong(colModifiedDateIndex) != -1) {
				...
				if (mState == STATE_EDIT) {
					...
					String text = String.format(
							res.getString(R.string.title_edit), title);
					setTitle(text);
			...
			if (!mInserted) {
				...
				Uri insertUri = Uri
						.parse("content://com.google.provider.NotePad/"
								+ mAccountName + "/notes/");
				ContentValues values = new ContentValues();
	...
	private void requestSync() {
		if (mAccountName != null && mAccountName.length() > 0) {
			final GoogleAccountManager accountManager = new GoogleAccountManager(
					this);
			...
			if (account != null) {
				...
				ContentResolver.requestSync(account,
						"com.google.provider.NotePad", options);
			}
		}
	}
