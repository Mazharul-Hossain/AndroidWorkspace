...
public class Preferences extends PreferenceActivity {
	...
	public static class PreferencesFragment extends PreferenceFragment {
		...
		public void onCreate(Bundle savedInstanceState) {
			...
			mAccountPreference = this
					.findPreference("selected_account_preference");
			mAccountPreference
			...
			mSyncPreference = (ListPreference) this
					.findPreference("sync_frequency_preference");
			mSyncPreference
					.setOnPreferenceChangeListener(new OnPreferenceChangeListener() {
						...
						public boolean onPreferenceChange(
							...
							SharedPreferences.Editor editor = PreferenceManager
									.getDefaultSharedPreferences(getActivity())
									.edit();
							editor.putString("sync_frequency_preference",
									(String) newValue);
							editor.commit();
		...
		/**
		 * Start an intent to prompt the user to choose the account to use with
		 * the app.
		 */
		private void chooseAccount() {
			mState = STATE_CHOOSING_ACCOUNT;
			Intent intent = AccountPicker.newChooseAccountIntent(
					getPreferenceAccount(), null, ACCOUNT_TYPE, false, null,
					null, null, null);
			...
			if (account != null) {
				...
				if (oldAccount != null) {
					ContentResolver.setSyncAutomatically(oldAccount,
							"com.google.provider.NotePad", false);
				}
				SharedPreferences.Editor editor = PreferenceManager
		 ...
		 * 
		 * @param account
		 *            Account to set sync frequency for.
		 */
		private void setSyncFrequency(Account account) {
			if (account != null) {
				String syncValue = mPreferences.getString(
						"sync_frequency_preference", "900");

				mSyncPreference.setSummary("Sync every "
						+ mSyncPreference.getEntries()[mSyncPreference
								.findIndexOfValue(syncValue)]);

				ContentResolver.setSyncAutomatically(account,
						"com.google.provider.NotePad", true);
				ContentResolver.addPeriodicSync(account,
						"com.google.provider.NotePad", new Bundle(),
		...
		private Account getPreferenceAccount() {
			return mAccountManager.getAccountByName(mPreferences.getString(
